...
同じルールで「黙らせる」を何度もやっていると、それは 3 つ目を選ぶべきサインである。chaff がそれを見つけて教えてくれる。
...
`chaff-spec.md` は「どう作るか」を定めている。rule の四層モデル、Plugin API、LanguageAdapter、finding format がそれにあたる。
...
chaff は (c) を一級の応答として設計する。§7 がこの spec の中核である。
...
①から④までを一度も止まらずに通せることを最優先とする。②を飛ばして④まで行けることも要件に入れている（§5）。
...
閾値は既定で 5 件としている。`suppressions.warn_threshold` で変更できる。
...
実務ではここが最大の壁になる。記事が 200 本ある repo に入れると 4000 件出る。全部直してから導入することは誰にもできない。
...
最後の行が重要である。マッチ 0 件を成功として返すと、「CI は通っているが何も検証していない」状態が延々と続く。
