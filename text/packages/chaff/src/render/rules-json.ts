...
const now = (rule: RuleDefinition, config: Config): Record<string, unknown> => {
  const explicit = config.rules[rule.id];
  if (explicit === "off") return { level: "off", why_off: "設定で止めている" };
  if (explicit !== undefined) return { level: explicit, limit: resolve(rule, explicit).limit };
  ...
  return { level: "normal", limit: resolve(rule, "normal").limit };
};
