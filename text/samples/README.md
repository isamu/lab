...
機械で判定できるものを AI に聞かない。これが仕様の最重要原則です（[nlh spec §22](../natural-language-validation-harness-spec.md) の deterministic first）。
...
`rules-for-ai.json` の「AI」は判定ではなく**設定を書かせる相手**のこと。軸 1 の AI とは別物です。
...
**値を列挙してあります**。`strict / normal / relaxed / off` の 4 つ。自由記述の数値より生成が安定する。
...
1 は精度が読めず、2 は §14 の原則を実質的に緩める。**未解決です**（spec §26-7 に記載）。
