...
アプリも、テストも、モデルも動かしません。ミューテーションテスト・Playwright・axe・Lighthouse・
AI レビューはいずれも当初の要望に入っていましたが、すべて理由付きで見送っています。
[docs/the-original-list.md](https://github.com/isamu/lab/blob/main/coding/docs/the-original-list.md)
に、当初挙がった項目の1つ1つがどうなったかを記録してあります。
...
リポジトリに [`repo.json`](https://github.com/repos-json/repos-json) —
「このリポジトリが何であるか」を宣言する共通のファイル — があれば、scoria はそこから3つを読みます。
同じことを2回書かなくて済みます。
...
- **`projects`** が測る対象になります。単位を宣言しているモノレポなら、scoria の設定は不要です。
- **`name`** がレポートとバッジでの呼び名になります。優先順位は、そのディレクトリ自身の `repo.json`、
  親の該当エントリ、そこの `package.json`、最後にディレクトリ名。
  **リポジトリ自身の名前は継承しません** — 5つのパッケージが全部「acme platform」と表示されるより、
  ディレクトリ名のほうがましだからです（§10.1）。
- **`extensions.scoria`** に設定を置けます。違うところだけ書けば、残りは自動検出されます。
  起動したルートの設定は各プロジェクトに引き継がれるので、`mode` は1回書けば済みます。

`scoria.config.json` があればそちらが勝ちます。`repo.json` は「リポジトリがすべてのツールに言うこと」で、
「あなたがこのツールに言ったこと」を上書きするものではないからです（§10）。

**`color` は意図的に読みません。** バッジの色は baseline からの増減を表しているので、
リポジトリ側から色を渡せると、自分の劣化を緑に塗れてしまいます。

### モノレポと、複数プロジェクトを含むリポジトリ

scoria が測るのは**指定された1ディレクトリだけ**です。勝手に下には潜りません。

scoria が動かすツールはどれも「1つのルート」に紐づいています。`tsc` は渡された tsconfig を読み、
`audit` は1つの lockfile を読み、`knip` は1つの依存グラフを解決します。
2つのプロジェクトを含むリポジトリに向けると、**ツール系は外側しか見ないのに、ファイル系は両方を数えます**。
分母は全体、分子は半分。いちばん悪い組み合わせです。

ownplate がまさにこの形でした。`src/` はルートの tsconfig と lockfile を使う Vue アプリ、
`functions/` は独自の tsconfig・package.json・**独自の yarn.lock** を持つ Firebase Functions です。
ルートから測ると `tsc` は「エラー0件」と言いましたが `functions/` の93ファイルを一度も見ておらず、
`audit` は `functions/` の high 4件・moderate 19件を丸ごと見落としていました。

なので scoria は境界を推測しません。**設定に書いてください。**
...
各 target は「そこで scoria を動かしたのと同じ」ように測ります。設定も baseline も target ごとです。
`targets` を書かなければ、指定した1ディレクトリだけを測ります — いま動いている設定の意味は変わりません。

**この書き方の規則は scoria のものではなく [`repo.json` §9](https://github.com/repos-json/repos-json) のものです。**
リポジトリが単位を一度宣言したら、それを読むどのツールも同じ集合を得られるようにするためです。

- `.` はリポジトリルートを指します。`[".", "functions"]` が ownplate の形です。
- ワイルドカードは「segment がちょうど `*`」のときだけです。`**` や `we*` のような部分一致は
  この版の仕様では未定義なので、**推測せずに拒否して理由を出します**。
- ディレクトリにのみマッチし、`.` で始まる名前・`node_modules`・`vendor` にはマッチしません。
- 並び順は UTF-16 code unit 順です（ロケール順ではありません）。
  同じパッケージ群を2つのツールが別の順序で並べるのを防ぐための規則です。
- 書いた順序は保たれます。**1ディレクトリ＝1プロジェクト**で、最初に指したエントリが位置を取ります。
- **ある target の範囲は、そのディレクトリから入れ子の target を引いたものです。**
  これが無いと `[".", "functions"]` は `functions/` を二重に数えます。
  scoria は自分のファイル収集から除くだけでなく、ディレクトリ全体を見るツール
  （oxlint・jscpd・madge）にも除外を渡します。

落としたものは理由付きで必ず報告します。何も解決しなければ**実行を失敗させます** —
0ディレクトリを測って成功と報告するのは、間違いに誰も気づけない唯一の結末だからです。

target が複数あるときは、`--sarif` と `--badge-json` の出力先にディレクトリ名が入ります
（`scoria.sarif` → `scoria.web.sarif`）。互いに上書きしないためです。

### 下がったら CI を落とす

既定では scoria は何もゲートしません。レポートを出して exit 0 で終わります。
入れた初日に CI が真っ赤になるツールは、誰も使い続けないからです。
...
これ以降、次のどれかが起きると exit 1 になります。

- どれかの観点が baseline より 1 点を超えて下がった
- 理由の書かれていない抑制が増えた
- `error` の指摘が、それまで無かったファイルに出た

**何をゲートしないかも同じくらい大事です。** 次の3つは「下がったこと」は表示しますが、
CI は落としません。出力の「下がっていますが、ゲートはしていません」に理由付きで並びます。

|                                     | 理由                                                                                                  |
| ----------------------------------- | ----------------------------------------------------------------------------------------------------- |
| confidence が `low`                 | その数字はコードではなく測定のほうの話だから                                                          |
| ツールの版数が変わった              | lint を上げればルールが増えるだけで、欠陥が増えたわけではない。ここで落とすと「上げない文化」ができる |
| リポジトリの規模が 20% 以上変わった | 密度の指標が別の分母を測っているから                                                                  |

黙って除外すると「下がらなかった」のと見分けがつかなくなるので、必ず表示します。

`mode` はコマンドのフラグではなく、コミットされる設定ファイルに書きます。
ゲートを入れるかどうかは出力ではなく方針なので、diff に出てレビューされるべきだからです。
...
先頭には棒グラフが出ます。GitHub は Markdown を描画する場所ならどこでも mermaid を描画するので、
画像も artifact も外部サービスも要りません。棒が今回、折れ線が baseline です
（同じ指標を測っていたときだけ引きます）。表はその下に残します —
グラフは confidence も「なぜ測れなかったか」も運べないからです。
